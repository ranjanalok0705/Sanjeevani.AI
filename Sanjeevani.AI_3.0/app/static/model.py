...
#     words = user_input.lower().split()  # Ensure each word is handled separately
#     user_tfidf = vectorizer.transform(words)  # Vectorize all words separately
    
#     # Compute max similarity per NGO (instead of biased to one category)
#     similarities = cosine_similarity(user_tfidf, tfidf_matrix).max(axis=0)
...
def predict_ngos(user_input, top_n=10, threshold=70):
    ...
    if matched_categories:
        ...
        return matching_ngos[['NGO Name', 'City', 'Contact', 'Email', 'category']]
    ...
    return closest_ngos[['NGO Name', 'City', 'Contact', 'Email', 'category']]
